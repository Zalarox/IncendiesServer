			...
			if (!p.isBare()) {
				...
				if (p.getSize() != Packet.Size.Fixed) {
					if (p.getSize() == Packet.Size.VariableByte) {
						if (dataLength > 255) // trying to send more data then
												// we can represent with 8 bits!
							throw new IllegalArgumentException(
									"Tried to send packet length " + dataLength + " in 8 bits [pid=" + p.getId() + "]");
						buffer.put((byte) dataLength);
					} else if (p.getSize() == Packet.Size.VariableShort) {
						if (dataLength > 65535) // trying to send more data then
												// we can represent with 16
												// bits!
