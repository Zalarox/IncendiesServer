...
public class Bushes {
	...
	public void doCalculations() {
		for (int i = 0; i < bushesSeeds.length; i++) {
			if (bushesStages[i] > 0 && bushesStages[i] <= 3 && GameEngine.getMinutesCounter() - bushesTimer[i] >= 5) {
				bushesStages[i]--;
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				bushesTimer[bushesFieldsData.getBushesIndex()] = GameEngine.getMinutesCounter();
				updateBushesStates();
				if (bushesStages[bushesFieldsData.getBushesIndex()] == 3) {
					container.stop();
					return;
