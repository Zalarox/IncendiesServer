...
public class ToolLeprechaun {
	...
	public Item[] storeItems = { new Item(5341), new Item(5343), new Item(952), new Item(5329), new Item(5331),
			new Item(5325) };
	public Item[] storeItems2 = { new Item(1925), new Item(6032), new Item(6034) };
	public Item[] storeItemsClient = { new Item(5341), new Item(5343), new Item(952), new Item(5329), new Item(5331),
			new Item(5325) };
	public Item[] storeItems2Client = { new Item(1925), new Item(6032), new Item(6034) };

	/* setting up the main constant field */

	public static final int[] NOTABLE_ITEMS = { 225, 239, 247, 249, 251, 253, 255, 257, 259, 261, 263, 265, 267, 269,
			3000, 2481, 592, 1965, 1967, 6004, 5980, 5976, 1955, 1963, 2108, 5972, 2114, 754, 2126, 248, 1951, 240,
			2367, 1942, 1957, 1965, 1982, 5986, 5504, 5982, 6006, 5994, 5996, 5931, 5998, 6000, 6002, 6016, 6055 };
	...
	public enum ToolStoreData {
		...
		RAKE(0, 5341, 1, 1, 15596, 15597, "Rake"), SEED_DIBBER(1, 5343, 2, 1, 15598, 15599, "Dibber"), SPADE(2, 952, 4,
				1, 15600, 15601, "Spade"), SECATEURS(3, 5329, 8, 1, 15602, 15603, "Secateurs"), MAGIC_SECATEURS(4, 7409,
						8, 1, 15602, 15603,
						"Secateurs"), WATERING_CAN_0(5, 5331, 16, 1, 15604, 15605, "Watering Can"), WATERING_CAN_1(6,
								5333, 32, 1, 15604, 15605, "Watering Can"), WATERING_CAN_2(7, 5334, 48, 1, 15604, 15605,
										"Watering Can"), WATERING_CAN_3(8, 5335, 64, 1, 15604, 15605,
												"Watering Can"), WATERING_CAN_4(9, 5336, 80, 1, 15604, 15605,
														"Watering Can"), WATERING_CAN_5(10, 5337, 96, 1, 15604, 15605,
																"Watering Can"), WATERING_CAN_6(11, 5338, 112, 1, 15604,
																		15605, "Watering Can"), WATERING_CAN_7(12, 5339,
																				128, 1, 15604, 15605,
																				"Watering Can"), WATERING_CAN_8(13,
																						5340, 144, 1, 15604, 15605,
																						"Watering Can"), GARDENING_TROWEL(
																								14, 5325, 256, 1, 15606,
																								15607,
																								"Trowel"), EMPTY_BUCKETS(
																										15, 1925, 512,
																										31, 15608,
																										15609,
																										"Buckets"), COMPOST(
		...
		ToolStoreData(int toolIndex, int toolId, int toolConfig, int toolMaxQuantity, int toolFrameId,
				int toolCountFrameId, String toolName) {
	...
	public void loadInterfaces() {
		player.getPA().sendInterface(LEPRECHAUN_INTERFACE);
		player.getPA().sendUpdateItems(LEPRECHAUN_INTERFACE_CONTAINER, storeItems);
		player.getPA().sendUpdateItems(LEPRECHAUN_INTERFACE_CONTAINER2, storeItems2);
		player.getPA().sendSidebarInterface(3, PLAYER_INTERFACE);
		player.getPA().sendUpdateItems(PLAYER_INTERFACE_CONTAINER, storeItemsClient);
		player.getPA().sendUpdateItems(PLAYER_INTERFACE_CONTAINER2, storeItems2Client);
		updateStore();
	}
	...
	public void updateStore() {
		...
		for (int i = 0; i < tools.length; i++) {
			...
			updateClientInterface(toolStoreData, player.getInventory().getItemAmount(toolStoreData.getToolId()), i);
			if (toolStoreData.getToolId() != 5332 && toolStoreData.getToolId() >= 5331
	...
	public void updateClientInterface(ToolStoreData toolStoreData, int count, int index) {
		...
		if (count > 0) {
			...
			// secateurs
			if ((index == 3 || index == 4) && (player.getInventory().getItemContainer().contains(7409)
					|| player.getInventory().getItemContainer().contains(5329))) {
				return;
			}

			player.getPA().sendString("" + toolStoreData.getToolName(), toolStoreData.getToolFrameId());
			player.getPA().sendString("" + count, toolStoreData.getToolCountFrameId());
	...
	public void storeItems(int itemId, int amount) {
		...
		if (toolStoreData.getToolMaxQuantity() == storeAmount
				|| (itemId == 7409 || itemId == 5329) && (tools[3] == 1 || tools[4] == 1)
				|| hasWateringCanInStore() && toolStoreData.getToolId() != 5332 && itemId >= 5340 && itemId <= 5331) {
			player.getPA().sendMessage("You can't store any more of those.");
			return;
		}
		if (player.getInventory().getItemContainer().getCount(itemId) <= 0) {
			player.getPA().sendMessage("You aren't carrying any of those.");
			return;
		}
	...
	public void withdrawItems(int itemId, int amount) {
		...
		if (tools[toolStoreData.getToolIndex()] <= 0) {
			player.getPA().sendMessage("You haven't got any of those stored in here.");
			return;
		}
		int finalAmount;
	...
	public boolean noteItem(int itemId) {
		...
		if (new Item(itemId).getDefinition().isNoted()) {
			player.getDialogue().sendNpcChat1("That IS a banknote!", 0, "Farming");
		...
		player.getDialogue().sendNpcChat1("Nay, I've got no banknotes to exchange for that item.", 0, "Farming");
