...
public class ChopTree {
	...
	static {
		...
		trees[1] = new Tree(new int[] { 1276, 1277, 1278, 1279, 1280, 1282, 1283, 1284, 1285, 1286, 1289, 1290, 1291,
				1315, 1316, 1318, 1319, 1330, 1331, 1332, 1333, 1365, 1383, 1384, 2409, 3033, 3034, 3035, 3036, 3881,
				3882, 3883, 5902, 5903, 5904 }, 1, 25, 1511, 1342, 75, 100);
	...
	public static boolean handleNest(Player player, int itemId) {
		...
		player.getPA().sendMessage("You search the nest...and find something in it!");
		player.getInventory().removeItem(new Item(itemId));
		player.getInventory().addItem(new Item(5075));
	...
	public static int getAxeIndex(Player player) {
		...
		for (int i = axes.length - 1; i >= 0; i--) {
			if (player.getInventory().getItemContainer().contains(axes[i].getId())) {
				if (player.getSkill().getLevel()[Skill.WOODCUTTING] >= axes[i].getLevel()) {
					return i;
				}
			}
		}
