...
public class Allotments {
	...
	public enum AllotmentData {
		...
		POTATO(5318, 1942, 5096, 3, 1, new int[] { 6032, 2 }, 40, 0.30, 8, 9.5, 0x06, 0x0c), ONION(5319, 1957, 5096, 3,
				5, new int[] { 5438, 1 }, 40, 0.30, 9.5, 10.5, 0x0d,
				0x13), CABBAGE(5324, 1965, 5097, 3, 7, new int[] { 5458, 1 }, 40, 0.25, 10, 11.5, 0x14, 0x1a), TOMATO(
						5322, 1982, 5096, 3, 12, new int[] { 5478, 2 }, 40, 0.25, 12.5, 14, 0x1b, 0x21), SWEETCORN(5320,
								5986, 6059, 3, 20, new int[] { 5931, 10 }, 40, 0.20, 17, 19, 0x22,
								0x2a), STRAWBERRY(5323, 5504, -1, 3, 31, new int[] { 5386, 1 }, 40, 0.20, 26, 29, 0x2b,
										0x33), WATERMELON(5321, 5982, 5098, 3, 47, new int[] { 5970, 10 }, 40, 0.20,
												48.5, 54.5, 0x34, 0x3e);
	...
	public enum AllotmentFieldsData {
		...
		CATHERBY_NORTH(0, new Point[] { new Point(2805, 3466), new Point(2806, 3468), new Point(2805, 3467),
				new Point(2814, 3468) }, 2324),

		CATHERBY_SOUTH(1, new Point[] { new Point(2805, 3459), new Point(2806, 3461), new Point(2802, 3459),
				new Point(2814, 3460) }, 2324),

		FALADOR_NORTH_WEST(2, new Point[] { new Point(3050, 3307), new Point(3051, 3312), new Point(3050, 3311),
				new Point(3054, 3312) }, 2323),

		FALADOR_SOUTH_EAST(3, new Point[] { new Point(3055, 3303), new Point(3059, 3304), new Point(3058, 3303),
				new Point(3059, 3308) }, 2323),

		PHASMATYS_NORTH_WEST(4, new Point[] { new Point(3597, 3525), new Point(3598, 3530), new Point(3597, 3529),
				new Point(3601, 3530) }, 2326),

		PHASMATYS_SOUTH_EAST(5, new Point[] { new Point(3602, 3521), new Point(3606, 3522), new Point(3605, 3521),
				new Point(3606, 3526) }, 2326),

		ARDOUGNE_NORTH(6, new Point[] { new Point(2662, 3377), new Point(2663, 3379), new Point(2662, 3378),
				new Point(2671, 3379) }, 2325),

		ARDOUGNE_SOUTH(7, new Point[] { new Point(2662, 3370), new Point(2663, 3372), new Point(2662, 3370),
			...
			for (AllotmentFieldsData allotmentFieldsData : AllotmentFieldsData.values()) {
				if (FarmingConstants.inRangeArea(allotmentFieldsData.getAllotmentPosition()[0],
						...
						|| FarmingConstants.inRangeArea(allotmentFieldsData.getAllotmentPosition()[2],
								allotmentFieldsData.getAllotmentPosition()[3], x, y)) {
	...
	public enum InspectData {
		POTATOES(5318,
				new String[][] { { "The potato seeds have only just been planted." },
				...
				{ "The potato plants are ready to harvest. A white",
						"flower at the top of each plant opens up." } }), ONIONS(
								5319,
								new String[][] { { "The onion seeds have only just been planted." },
								...
								{ "The onion stalks are larger than before and the",
										"onion is now light and brown coloured." } }), CABBAGES(
												5324,
												new String[][] { { "The cabbage seeds have only just been planted,",
	...
	public boolean waterPatch(int objectX, int objectY, int itemId) {
		...
		if (allotmentState[allotmentFieldsData.getAllotmentIndex()] == 1
				...
				|| allotmentStages[allotmentFieldsData.getAllotmentIndex()] == allotmentData.getEndingState()
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				allotmentTimer[allotmentFieldsData.getAllotmentIndex()] = GameEngine.getMinutesCounter();
				updateAllotmentsStates();
				if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] != 3) {
			player.getPA().sendMessage("You can't plant a seed here.");
			return false;
		}
		if (!Constants.FARMING_ENABLED) {
			player.getPA().sendMessage("This skill is currently disabled.");
		...
		if (allotmentData.getLevelRequired() > player.getSkill().getLevel()[Skill.FARMING]) {
			player.getDialogue().sendStatement(
					"You need a farming level of " + allotmentData.getLevelRequired() + " to plant this seed.");
			return true;
		}
		...
		if (player.getInventory().getItemAmount(allotmentData.getSeedId()) < allotmentData.getSeedAmount()) {
			player.getDialogue()
					.sendStatement("You need atleast " + allotmentData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
	...
	private void displayAll() {
		for (int i = 0; i < allotmentStages.length; i++) {
			...
			System.out.println("level : " + allotmentStages[i]);
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] == 0) {
					allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] = 1
							+ (START_HARVEST_AMOUNT + Misc.random(END_HARVEST_AMOUNT - START_HARVEST_AMOUNT)) * (1);
				}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
			player.getDialogue().sendStatement("This is an allotment patch. The soil has not been treated.",
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				@Override
				public void execute(CycleEventContainer container) {
					if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4 < inspectData.getMessages().length
							- 2) {
						player.getDialogue().sendStatement(inspectData
								.getMessages()[allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
					} else if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] < allotmentData.getEndingState()
	...
	public void handleFlowerProtection(int index) {
		...
		if (!allotmentWatched[index]) {// Misc.random(100) <= maxChance) {
			...
			if (player.getFlowers().flowerState[indexGiven] != 3 && player.getFlowers().hasFullyGrown[indexGiven]
					&& player.getFlowers().flowerSeeds[indexGiven] == allotmentData.getFlowerProtect()) {
