...
public class Herbs {
	...
	public enum HerbData {
		GUAM(5291, 199, 9, 80, 0.25, 11, 12.5, 0x04, 0x08), MARRENTILL(5292, 201, 14, 80, 0.25, 13.5, 15, 0x0b,
				0x0f), TARROMIN(5293, 203, 19, 80, 0.25, 16, 18, 0x12, 0x16), HARRALANDER(5294, 205, 26, 80, 0.25, 21.5,
						24, 0x19, 0x1d), GOUT_TUBER(6311, 3261, 29, 80, 0.25, 105, 45, 0xc0, 0xc4), RANARR(5295, 207,
								32, 80, 0.20, 27, 30.5, 0x20,
								0x24), TOADFLAX(5296, 3049, 38, 80, 0.20, 34, 38.5, 0x27, 0x2b), IRIT(5297, 209, 44, 80,
										0.20, 43, 48.5, 0x2e,
										0x32), AVANTOE(5298, 211, 50, 80, 0.20, 54.5, 61.5, 0x35, 0x39), KUARM(5299,
												213, 56, 80, 0.20, 69, 78, 0x44, 0x48), SNAPDRAGON(5300, 3051, 62, 80,
														0.15, 87.5, 98.5, 0x4b, 0x4f), CADANTINE(5301, 215, 67, 80,
																0.15, 106.5, 120, 0x52, 0x56), LANTADYME(5302, 2485, 73,
																		80, 0.15, 134.5, 151.5, 0x59, 0x5d), DWARF(5303,
																				217, 79, 80, 0.15, 170.5, 192, 0x60,
																				0x64), TORSOL(5304, 219, 85, 80, 0.15,
	...
	public enum HerbFieldsData {
		ARDOUGNE(0, new Point[] { new Point(2670, 3374), new Point(2671, 3375) }), PHASMATYS(1,
				new Point[] { new Point(3605, 3529), new Point(3606, 3530) }), FALADOR(2,
						new Point[] { new Point(3058, 3311), new Point(3059, 3312) }), CATHERBY(3,
								new Point[] { new Point(2813, 3463), new Point(2814, 3464) });
	...
	public enum InspectData {
		...
		GUAM(5291,
				new String[][] { { "The seed has only just been planted." }, { "The herb is now ankle height." },
						{ "The herb is now knee height." }, {
								"The herb is now mid-thigh height." },
				{ "The herb is fully grown and ready to harvest." } }), MARRENTILL(
						5292,
						new String[][] { { "The seed has only just been planted." }, {
								"The herb is now ankle height." }, { "The herb is now knee height." },
						{ "The herb is now mid-thigh height." },
	...
	public void updateHerbsStates() {
		...
		player.getPA().sendConfig(MAIN_HERB_LOCATION_CONFIG, configValue);
	...
	public int getConfigValue(int herbStage, int seedId, int plantState, int index) {
		...
		return (plantState == 2 ? herbStages[index] + 0x9e
				: plantState == 1 ? herbStages[index] + 0x9a : getPlantState(plantState) << 6)
	...
	public void doCalculations() {
		for (int i = 0; i < herbSeeds.length; i++) {
			if (herbStages[i] > 0 && herbStages[i] <= 3 && GameEngine.getMinutesCounter() - herbTimer[i] >= 5) {
				herbStages[i]--;
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (herbHarvest[herbFieldsData.getHerbIndex()] == 0) {
					herbHarvest[herbFieldsData.getHerbIndex()] = 1
				...
				player.getPA().sendMessage("You harvest the crop, and get some herbs.");
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (herbStages[herbFieldsData.getHerbIndex()] == 0) {
			player.getDialogue().sendStatement("This is an herb patch. The soil has not been treated.",
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (herbStages[herbFieldsData.getHerbIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDialogue().sendStatement(
								inspectData.getMessages()[herbStages[herbFieldsData.getHerbIndex()] - 4]);
					} else if (herbStages[herbFieldsData.getHerbIndex()] < herbData.getEndingState()
