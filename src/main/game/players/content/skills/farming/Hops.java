...
public class Hops {
	...
	public enum HopsData {
		BARLEY(5305, 6006, 4, 3, new int[] { 6032, 3 }, 40, 0.35, 8.5, 9.5, 0x31, 0x35), HAMMERSTONE(5307, 5994, 4, 4,
				new int[] { 6010, 1 }, 40, 0.35, 9, 10, 0x04,
				0x08), ASGARNIAN(5308, 5996, 4, 8, new int[] { 5458, 1 }, 40, 0.30, 10.5, 12, 0x0b, 0x10), JUTE(5306,
						5931, 3, 13, new int[] { 6008, 6 }, 40, 0.30, 13, 14.5, 0x38, 0x3d), YANILLIAN(5309, 5998, 4,
								16, new int[] { 5968, 1 }, 40, 0.25, 14.5, 16, 0x13, 0x19), KRANDORIAN(5310, 6000, 4,
										21, new int[] { 5478 }, 40, 0.25, 17.5, 19.5, 0x1c, 0x23), WILDBLOOD(5311, 6002,
												4, 28, new int[] { 6012, 1 }, 40, 0.20, 23, 26, 0x26, 0x2e),;
	...
	public enum HopsFieldsData {
		...
		LUMBRIDGE(0, new Point[] { new Point(3227, 3313), new Point(3231, 3317) }, 2333), MCGRUBOR(1,
				new Point[] { new Point(2664, 3523), new Point(2669, 3528) }, 2334), YANILLE(2,
						new Point[] { new Point(2574, 3103), new Point(2577, 3106) },
						2332), ENTRANA(3, new Point[] { new Point(2809, 3335), new Point(2812, 3338) }, 2327);
	...
	public enum InspectData {
		...
		BARLEY(5305,
				new String[][] { { "The barley seeds have only just been planted." },
						...
						{ "The barley is ready to harvest. The heads of grain",
								"are weighing down heavily on the stalks!" } }), HAMMERSTONE(
										5307,
										new String[][] { { "The Hammerstone seeds have only just been planted." },
	...
	public void doCalculations() {
		for (int i = 0; i < hopsSeeds.length; i++) {
			if (hopsStages[i] > 0 && hopsStages[i] <= 3 && GameEngine.getMinutesCounter() - hopsTimer[i] >= 5) {
				hopsStages[i]--;
	...
	public boolean waterPatch(int objectX, int objectY, int itemId) {
		...
		if (hopsState[hopsFieldsData.getHopsIndex()] == 1 || hopsStages[hopsFieldsData.getHopsIndex()] <= 1
				|| hopsStages[hopsFieldsData.getHopsIndex()] == hopsData.getEndingState() - hopsData.getStartingState()
						+ 4) {
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				hopsTimer[hopsFieldsData.getHopsIndex()] = GameEngine.getMinutesCounter();
				updateHopsStates();
				if (hopsStages[hopsFieldsData.getHopsIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (player.getInventory().getItemAmount(hopsData.getSeedId()) < hopsData.getSeedAmount()) {
			player.getDialogue()
					.sendStatement("You need atleast " + hopsData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (hopsHarvest[hopsFieldsData.getHopsIndex()] == 0) {
					hopsHarvest[hopsFieldsData.getHopsIndex()] = 1
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (hopsStages[hopsFieldsData.getHopsIndex()] == 0) {
			player.getDialogue().sendStatement("This is a hops patch. The soil has not been treated.",
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					} else if (hopsStages[hopsFieldsData.getHopsIndex()] < hopsData.getEndingState()
