...
public class Flowers { // todo scarecrow 6059
	...
	public void doCalculations() {
		for (int i = 0; i < flowerSeeds.length; i++) {
			if (flowerStages[i] > 0 && flowerStages[i] <= 3 && GameEngine.getMinutesCounter() - flowerTimer[i] >= 5) {
				flowerStages[i]--;
				flowerTimer[i] = GameEngine.getMinutesCounter();
				updateFlowerStates();
			}
			if (GameEngine.getMinutesCounter() - flowerTimer[i] >= 5 && flowerSeeds[i] > 0x21
					&& flowerSeeds[i] <= 0x24) {
				flowerSeeds[i]--;
				updateFlowerStates();
				return;
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				flowerTimer[flowerFieldsData.getFlowerIndex()] = GameEngine.getMinutesCounter();
				updateFlowerStates();
				if (flowerStages[flowerFieldsData.getFlowerIndex()] == 3) {
					container.stop();
					return;
