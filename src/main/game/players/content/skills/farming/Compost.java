...
import main.util.GlobalToolConstants;
...
public class Compost {
	...
	public enum CompostBinLocations {
		NORTH_ARDOUGNE(0, FIRST_TYPE_COMPOST_BIN, 3, 2661, 3375), PHASMATYS(1, SECOND_TYPE_COMPOST_BIN, 1, 3610,
				3522), FALADOR(2, FIRST_TYPE_COMPOST_BIN, 4, 3056, 3312), CATHERBY(3, FIRST_TYPE_COMPOST_BIN, 3, 2804,
						3464);
	...
	public enum CompostBinStages {
		FIRST_TYPE(7808, 7813, 7809, 7810, 7811, 7812, 7814, 7815, 7816, 7817, 7828, 7829, 7830,
				7831), SECOND_TYPE(7818, 7823, 7819, 7820, 7821, 7822, 7824, 7825, 7826, 7827, 7832, 7833, 7834, 7835);
	...
	private void updateCompostBin(int index) {
		...
		player.getPA().sendObject(finalObject, x, y, z, CompostBinLocations.forId(index).getObjectFace(), 10);
	}
	...
	public void closeCompostBin(final int index) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				player.getPA().sendMessage("You close the compost bin, and its content start to rot.");
	...
	public void openCompostBin(final int index) {
		...
		if (GameEngine.getMinutesCounter() - compostBinsTimer[index] >= timerRequired) {
			...
			player.getPA().sendMessage("The compost bin is still rotting. I should wait until it is complete.");
		}
	}
	...
	public void fillCompostBin(int x, int y, final int organicItemUsed) {
		...
		if (incrementFactor == 0) {
			player.getPA().sendMessage("You need to put organic items into the compost bin in order to make compost.");
			return;
		}
		...
		player.setSkilling(new CycleEvent() {
			...
			@Override
			public void execute(CycleEventContainer container) {
				if (!player.checkTask(task) || !player.getInventory().getItemContainer().contains(organicItemUsed)
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (!player.checkTask(task)
						|| !player.getInventory().getItemContainer().contains(GlobalToolConstants.BUCKET)
				...
				player.getSkill().addExp(Skill.FARMING, finalItem == COMPOST ? COMPOST_EXP_RETRIEVE
						: finalItem == SUPER_COMPOST ? SUPER_COMPOST_EXP_RETRIEVE : ROTTEN_TOMATOES_EXP_RETRIEVE);
	...
	public boolean handleItemOnObject(int itemUsed, int objectId, int objectX, int objectY) {
		...
		case 7827:
			if (itemUsed == GlobalToolConstants.BUCKET) {
				...
				player.getPA().sendMessage("You might need some buckets to gather the compost.");
			}
			return true;
