...
import incendius.game.players.content.ItemsKeptOnDeath;
...
import incendius.game.players.content.skills.cooking.DairyChurn;
import incendius.game.players.content.skills.cooking.FlourRelated;
import incendius.game.players.content.skills.crafting.DramenBranch;
...
import incendius.game.players.content.skills.crafting.PotteryMaking;
import incendius.game.players.content.skills.crafting.SilverCrafting;
import incendius.game.players.content.skills.crafting.Spinning;
import incendius.game.players.content.skills.crafting.Tanning;
import incendius.game.players.content.skills.crafting.Weaving;
...
import incendius.game.players.content.skills.smithing.Smelting;
import incendius.game.players.content.skills.summoning.Creation;
	...
	public void processPacket(final Player p, int packetType, int packetSize) {
		...
		case 184151:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 0) {
							Server.clanChat.clans[j].whoCanEnterChat = 0;
		...
		case 187128:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 8) {
							Server.clanChat.clans[j].whoCanEnterChat = 8;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187129:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 7) {
							Server.clanChat.clans[j].whoCanEnterChat = 7;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187130:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 6) {
							Server.clanChat.clans[j].whoCanEnterChat = 6;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187131:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 5) {
							Server.clanChat.clans[j].whoCanEnterChat = 5;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187132:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 4) {
							Server.clanChat.clans[j].whoCanEnterChat = 4;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187133:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 3) {
							Server.clanChat.clans[j].whoCanEnterChat = 3;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187134:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 2) {
							Server.clanChat.clans[j].whoCanEnterChat = 2;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187135:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanEnterChat != 1) {
							Server.clanChat.clans[j].whoCanEnterChat = 1;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 184154:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 0) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 0;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187138:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 8) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 8;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187139:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 7) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 7;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187140:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 6) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 6;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187141:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 5) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 5;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187142:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 4) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 4;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187143:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 3) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 3;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187144:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 2) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 2;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187145:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanTalkOnChat != 1) {
							Server.clanChat.clans[j].whoCanTalkOnChat = 1;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 184157:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 8) {
							Server.clanChat.clans[j].whoCanKickOnChat = 8;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187149:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 7) {
							Server.clanChat.clans[j].whoCanKickOnChat = 7;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187150:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 6) {
							Server.clanChat.clans[j].whoCanKickOnChat = 6;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187151:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 5) {
							Server.clanChat.clans[j].whoCanKickOnChat = 5;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187152:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 4) {
							Server.clanChat.clans[j].whoCanKickOnChat = 4;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187153:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 3) {
							Server.clanChat.clans[j].whoCanKickOnChat = 3;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 187154:
			for (int j = 0; j < Server.clanChat.clans.length; j++) {
				if (Server.clanChat.clans[j] != null) {
					if (Server.clanChat.clans[j].owner.equalsIgnoreCase(p.playerName)) {
						if (Server.clanChat.clans[j].whoCanKickOnChat != 2) {
							Server.clanChat.clans[j].whoCanKickOnChat = 2;
							p.updateClanChatEditInterface(true);
							Server.clanChat.clans[j].changesMade = true;
						}
					}
		...
		case 193219:
			if (p.getInstance().clanId > -1) {
				Server.clanChat.leaveClan(p.playerId, p.getInstance().clanId, false);
			} else {
		...
		case 71074:
			Server.clanChat.handleShareButton(p);
			break;
