...
public class Smelting extends SkillHandler {
	...
	private static int COPPER = 436, TIN = 438, IRON = 440, COAL = 453, MITH = 447, ADDY = 449, RUNE = 451, GOLD = 444,
			SILVER = 442;
			...
			if (bartype == data[i][0]) {
				...
				if (data[i][3] > 0 && data[i][4] > 0) { // All OTHER bars
					if (!c.getItems().playerHasItem(data[i][3]) || !c.getItems().playerHasItem(data[i][4])) {
						c.sendMessage("You need an " + c.getItems().getItemName(data[i][3]).toLowerCase() + " and "
								+ data[i][5] + " " + c.getItems().getItemName(data[i][4]).toLowerCase()
								+ " to make this bar.");
				...
				if (data[i][4] < 0) { // Iron bar, Gold & Silver requirements
					if (!c.getItems().playerHasItem(data[i][3])) {
						c.sendMessage("You need an " + c.getItems().getItemName(data[i][3]).toLowerCase()
								+ " to make this bar.");
						c.getPA().removeAllWindows();
						return;
					}
				...
				if (data[i][5] > 0) { // Bars with more than 1 coal requirement
					if (!c.getItems().playerHasItem(data[i][4], data[i][5])) {
						c.sendMessage("You need an " + c.getItems().getItemName(data[i][3]).toLowerCase() + " and "
								+ data[i][5] + " coal ores to make this bar.");
						c.getPA().removeAllWindows();
						return;
					}
				}
