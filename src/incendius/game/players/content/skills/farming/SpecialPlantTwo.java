...
public class SpecialPlantTwo {
	...
	public enum SpecialPlantData {
		BELLADONNA(5281, 2398, 1, 63, 280, 0.15, 91, 512, 0x04, 0x08, -1, 0, 5, 8), CACTUS(5280, 6016, 1, 55, 550, 0.15,
				66.5, 25, 0x08, 0x12, 0x1f, 374, 11,
				17), BITTERCAP(5282, 6004, 1, 53, 220, 0.15, 61.5, 57.7, 0x04, 0x0f, -1, 0, 12, 17);
	...
	public enum SpecialPlantFieldsData {
		DRAYNOR_MANOR(0, new Point[] { new Point(3086, 3354), new Point(3087, 3355) }, 5281), AL_KARID(2,
				new Point[] { new Point(3315, 3202), new Point(3316, 3203) },
	...
	public enum InspectData {
		BELLADONNA(5281,
				new String[][] { { "The belladonna seed has only just been planted." },
						...
						{ "The belladonna plant is ready to harvest." } }), CACTUS(
								5280,
								new String[][] { { "The cactus seed has only just been planted." },
										...
										{ "The cactus is ready to be harvested." } }), BITTERCAP(
												5282,
												new String[][] { { "The mushroom spore has only just been planted." },
	...
	public void doCalculations() {
		for (int i = 0; i < specialPlantSeeds.length; i++) {
			...
			if (4 + state != specialPlantStages[i]
				...
				if (specialPlantStages[i] == specialPlantData.getEndingState() - specialPlantData.getStartingState() - 2
	...
	public boolean plantSeeds(int objectX, int objectY, final int seedId) {
		...
		if (player.getInventory().getItemAmount(specialPlantData.getSeedId()) < specialPlantData.getSeedAmount()) {
			player.getDialogue()
					.sendStatement("You need atleast " + specialPlantData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
	...
	public boolean harvestOrCheckHealth(int objectX, int objectY) {
		...
		final SpecialPlantData specialPlantData = SpecialPlantData
				.forId(specialPlantSeeds[specialPlantFieldsData.getSpecialPlantsIndex()]);
		if (specialPlantData == null) {
			return false;
		}
		if (!Constants.FARMING_ENABLED) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA()
						.sendMessage("You harvest the crop, and pick some " + specialPlantData.getHarvestId() + ".");
				...
				case BITTERCAP:
					...
					if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 16) {
						resetSpecialPlants(specialPlantFieldsData.getSpecialPlantsIndex());
						specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] = 3;
						specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Server
								.getMinutesCounter();
					}
					break;
				}
