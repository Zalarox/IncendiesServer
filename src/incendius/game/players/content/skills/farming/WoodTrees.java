...
import incendius.game.items.ItemDefinition;
import incendius.game.players.Player;
import incendius.game.players.content.skills.farming.ChopTree.Tree;
...
public class WoodTrees {
	...
	public enum TreeData {
		OAK(5370, 6043, 15, new int[] { 5968, 1 }, 160, 0.20, 14, 467.3, 0x08, 0x0c, 0x0d, 0x0e, 1281), WILLOW(5371,
				6045, 30, new int[] { 5386, 1 }, 240, 0.20, 25, 1456.3, 0x0f, 0x15, 0x16, 0x17, 1308), MAPLE(5372, 6047,
						45, new int[] { 5396, 1 }, 320, 0.25, 45, 3403.4, 0x18, 0x20, 0x21, 0x22, 1307), YEW(5373, 6049,
								60, new int[] { 6016, 10 }, 400, 0.25, 81, 7069.9, 0x23, 0x2d, 0x2e, 0x2f,
								1309), MAGIC(5374, 6051, 75, new int[] { 5976, 25 }, 480, 0.25, 145.5, 13768.3, 0x30,
										0x3c, 0x3d, 0x3e, 1292);
		...
		TreeData(int saplingId, int rootsId, int levelRequired, int[] paymentToWatch, int growthTime,
				double diseaseChance, double plantingXp, double checkHealthXp, int startingState, int endingState,
				int chopDownState, int stumpState, int treeObjectAssociated) {
	...
	public enum TreeFieldsData {
		VARROCK(0, new Point[] { new Point(3228, 3458), new Point(3230, 3460) }, 2341), LUMBRIDGE(1,
				new Point[] { new Point(3192, 3230), new Point(3194, 3232) }, 2342), TAVERLEY(2,
						new Point[] { new Point(2935, 3437), new Point(2937, 3439) },
						2339), FALADOR(3, new Point[] { new Point(3003, 3372), new Point(3005, 3374) }, 2340);
		...
		public static TreeFieldsData forIdPosition(int x, int y) {
			for (TreeFieldsData treeFieldsData : TreeFieldsData.values()) {
				if (FarmingConstants.inRangeArea(treeFieldsData.getTreePosition()[0],
	...
	public enum InspectData {
		OAK(5370,
				new String[][] { { "The acorn sapling has only just been planted." },
						{ "The acorn sapling grows larger." }, { "The oak tree produces a small canopy." }, {
								"The oak tree grows larger." },
				{ "The oak tree is ready to harvest." } }), WILLOW(
						5371,
						new String[][] { { "The willow sapling has only just been planted." },
								...
								{ "The trunk thickens, and the canopy grows", "yet larger." },
								{ "The willow tree is fully grown." } }), MAPLE(
										5372,
										new String[][] { { "The maple sapling has only just been planted." }, {
												"The mapling sapling grows a few small branches." },
										{ "The maple tree develops a small canopy." },
										{ "The maple tree trunk straightens and the canopy", "grows larger." },
										{ "The maple tree canopy grows." }, { "The maple tree grows." },
										{ "The maple tree is ready to be harvested." } }), YEW(
												5373,
												new String[][] { { "The yew sapling has only just been planted." },
										...
										{ "The yew tree is ready to be harvested." } }), MAGIC(
												5374,
												new String[][] { { "The magic sapling has only just been planted." },
												...
												{ "The magic tree grows taller." }, { "The magic tree grows taller." },
	...
	public void updateTreeStates() {
		...
		player.getPA().sendConfig(MAIN_TREE_CONFIG, configValue);
	...
	public void doCalculations() {
		for (int i = 0; i < treeSaplings.length; i++) {
			if (treeStages[i] > 0 && treeStages[i] <= 3 && Server.getMinutesCounter() - treeTimer[i] >= 5) {
				treeStages[i]--;
				treeTimer[i] = Server.getMinutesCounter();
				updateTreeStates();
				continue;
			}
			...
			long difference = Server.getMinutesCounter() - treeTimer[i];
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (treeStages[hopsFieldsData.getTreeIndex()] <= 2) {
					...
					player.getInventory().addItem(new Item(6055, 1));
				} else {
					treeStages[hopsFieldsData.getTreeIndex()] = 3;
					container.stop();
				}
				player.getSkill().addExp(Skill.FARMING, CLEARING_EXPERIENCE);
				treeTimer[hopsFieldsData.getTreeIndex()] = Server.getMinutesCounter();
				updateTreeStates();
				if (treeStages[hopsFieldsData.getTreeIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (treeStages[treeFieldsData.getTreeIndex()] != 3) {
			player.getPA().sendMessage("You can't plant a sapling here.");
			return true;
		}
		if (treeData.getLevelRequired() > player.getSkill().getLevel()[Skill.FARMING]) {
			player.getDialogue().sendStatement(
					"You need a farming level of " + treeData.getLevelRequired() + " to plant this sapling.");
			return true;
		}
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				treeTimer[treeFieldsData.getTreeIndex()] = Server.getMinutesCounter();
	...
	public boolean checkHealth(int objectX, int objectY) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA()
						.sendMessage("You examine the tree for signs of disease and find that it is in perfect health");
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (treeState[treeFieldsData.getTreeIndex()] == 1) {
			player.getDialogue().sendStatement("This tree is diseased. Use secateurs to prune the area, ",
					"or clear the patch with a spade.");
			return true;
		} else if (treeState[treeFieldsData.getTreeIndex()] == 2) {
			player.getDialogue().sendStatement("This tree is dead. You did not cure it while it was diseased.",
					"Clear the patch with a spade.");
			return true;
		} else if (treeState[treeFieldsData.getTreeIndex()] == 6) {
			player.getDialogue().sendStatement("This is a tree stump, to remove it, use a spade on it",
					"to recieve some roots and clear the patch.");
			return true;
		}
		if (treeStages[treeFieldsData.getTreeIndex()] == 0) {
			player.getDialogue().sendStatement("This is a tree patch. The soil has not been treated.",
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (treeStages[treeFieldsData.getTreeIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDialogue().sendStatement(
								inspectData.getMessages()[treeStages[treeFieldsData.getTreeIndex()] - 4]);
					} else if (treeStages[treeFieldsData.getTreeIndex()] < treeData.getEndingState()
							- treeData.getStartingState() + 2) {
						player.getDialogue()
	...
	public boolean pruneArea(int objectX, int objectY, int itemId) {
		...
		if (treeState[treeFieldsData.getTreeIndex()] != 1) {
			player.getPA().sendMessage("This area doesn't need to be pruned.");
			return true;
		}
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			@Override
			public void execute(CycleEventContainer container) {
				player.getPA().sendMessage("You prune the area with your secateurs.");
				container.stop();
			}
	...
	public boolean cut(final int x, final int y) {
		...
		if (ChopTree.getAxe(player) == null) {
			player.getPA().sendMessage("You do not have an axe which you have the woodcutting level to use.");
			return true;
		}

		final int object = treeData.getTreeObjectAssociated();
		player.getPA().sendMessage("You swing your axe at the tree.");
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getUpdateFlags().sendAnimation(emoteId);
				if (SkillHandler.skillCheck(player.getSkill().getLevel()[Skill.WOODCUTTING], treeLevel,
						axeChance * 20)) {
					player.getInventory().addItem(new Item(ChopTree.getTree(object).getLog()));
					player.getPA().sendMessage("You get some "
							+ ItemDefinition.forId(ChopTree.getTree(object).getLog()).getName().toLowerCase() + ".");
					player.getSkill().addExp(Skill.WOODCUTTING, ChopTree.getTree(object).getXP());
			...
			@Override
			public void stop() {
				// c.getPA().resetClientSkillVariables();
			}
		});
	...
	public boolean canCut(final int x, final int y) {
		...
		if (!SkillHandler.hasRequiredLevel(player, Skill.WOODCUTTING, ChopTree.getTree(object).getLevel(),
				"chop this tree")) {
			return false;
		}
		return true;
