...
public class Bushes {
	...
	public enum BushesData {
		REDBERRY(5101, 1951, 1, 10, new int[] { 5478, 4 }, 100, 0.20, 11.5, 4.5, 0x05, 0x0e, 0x09, 0x3a,
				64), CADAVABERRY(5102, 753, 1, 22, new int[] { 5968, 3 }, 140, 0.20, 18, 7, 0x0f, 0x19, 0x14, 0x3b,
						102.5), DWELLBERRY(5103, 2126, 1, 36, new int[] { 5406, 3 }, 140, 0.20, 31.5, 12, 0x1a, 0x25,
								0x20, 0x3c, 177.5), JANGERBERRY(5104, 247, 1, 48, new int[] { 5982, 6 }, 160, 0.20,
										50.5, 19, 0x26, 0x32, 0x2d, 0x3d, 284.5), WHITEBERRY(5105, 239, 1, 59,
												new int[] { 6004, 8 }, 160, 0.20, 78, 29, 0x33, 0x3f, 0x3a, 0x3e,
												437.5), POISONIVYBERRY(5106, 6018, 1, 70, null, 160, 0.20, 120, 45,
														0xc5, 0xd1, 0xcc, 0x3f, 674);
	...
	public enum BushesFieldsData {
		ETCETERIA(0, new Point[] { new Point(2591, 3863), new Point(2592, 3864) }, 2337), SOUTH_ARDOUGNE(1,
				new Point[] { new Point(2617, 3225), new Point(2618, 3226) }, 2338), CHAMPION_GUILD(2,
						new Point[] { new Point(3181, 3357), new Point(3182, 3358) },
						2335), RIMMINGTON(3, new Point[] { new Point(2940, 3221), new Point(2941, 3222) }, 2336);
	...
	public enum InspectData {
		...
		REDBERRY(5101,
				new String[][] { { "The Redberry seeds have only just been planted." },
						{ "The Redberry bush grows larger." },
						{ "The Redberry bush grows larger." }, { "The Redberry bush grows small, unripe,",
								"green berries." },
	...
	public void updateBushesStates() {
		...
		player.getPA().sendConfig(MAIN_BUSHES_CONFIG, configValue);
	}
	...
	public void doCalculations() {
		for (int i = 0; i < bushesSeeds.length; i++) {
			if (bushesStages[i] > 0 && bushesStages[i] <= 3 && Server.getMinutesCounter() - bushesTimer[i] >= 5) {
				bushesStages[i]--;
				bushesTimer[i] = Server.getMinutesCounter();
				updateBushesStates();
				continue;
			}
			...
			long difference = Server.getMinutesCounter() - bushesTimer[i];
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				bushesTimer[bushesFieldsData.getBushesIndex()] = Server.getMinutesCounter();
				updateBushesStates();
				if (bushesStages[bushesFieldsData.getBushesIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (player.getInventory().getItemAmount(bushesData.getSeedId()) < bushesData.getSeedAmount()) {
			player.getDialogue()
					.sendStatement("You need atleast " + bushesData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
	...
	public boolean harvestOrCheckHealth(int objectX, int objectY) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA().sendMessage("You harvest the crop, and pick some berries.");
				player.getInventory().addItem(new Item(bushesData.getHarvestId()));
				player.getSkill().addExp(Skill.FARMING, bushesData.getHarvestXp());
				bushesTimer[bushesFieldsData.getBushesIndex()] = Server.getMinutesCounter();
				int difference = bushesData.getEndingState() - bushesData.getStartingState();
				int growth = bushesData.getGrowthTime();
				lowerStage(bushesFieldsData.getBushesIndex(), growth
						- (growth / difference) * (difference + 5 - bushesStages[bushesFieldsData.getBushesIndex()]));
				modifyStage(bushesFieldsData.getBushesIndex());
				container.stop();
			}
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (bushesStages[bushesFieldsData.getBushesIndex()] == 0) {
			player.getDialogue().sendStatement("This is a bush patch. The soil has not been treated.",
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (bushesStages[bushesFieldsData.getBushesIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDialogue().sendStatement(
								inspectData.getMessages()[bushesStages[bushesFieldsData.getBushesIndex()] - 4]);
					} else if (bushesStages[bushesFieldsData.getBushesIndex()] < bushesData.getEndingState()
