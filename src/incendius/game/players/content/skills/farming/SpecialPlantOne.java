...
public class SpecialPlantOne {
	...
	public enum SpecialPlantFieldsData {
		BRIMHAVEN(0, new Point[] { new Point(2801, 3202), new Point(2803, 3204) }, 5375), KARAMJA(1,
				new Point[] { new Point(2795, 3100), new Point(2797, 3102) }, 5503), DRAYNOR(2,
						new Point[] { new Point(3059, 3257), new Point(3061, 3259) },
						5375), ETCETERIA(3, new Point[] { new Point(2612, 3857), new Point(2614, 3859) }, 5375);
		...
		public static SpecialPlantFieldsData forIdPosition(int x, int y) {
			for (SpecialPlantFieldsData specialPlantFieldsData : SpecialPlantFieldsData.values()) {
				if (FarmingConstants.inRangeArea(specialPlantFieldsData.getSpecialPlantPosition()[0],
						specialPlantFieldsData.getSpecialPlantPosition()[1], x, y)) {
	...
	public enum InspectData {
		...
		SPIRIT_TREE(5375,
				new String[][] { { "The spirit tree sapling has only just been planted. It has not grown yet." },
				...
				{ "The spirit tree canopy is smaller, the face is fully defined and the texture of the tree has changed dramatically",
						"The Spirit tree is ready to be checked." } }), CALQUAT_TREE(
								5503,
								new String[][] { { "The calquat sapling has only just been planted." },
	...
	public void updateSpecialPlants() {
		...
		player.getPA().sendConfig(MAIN_SPECIAL_PLANT_CONFIG, configValue);
	}
	...
	public void doCalculations() {
		for (int i = 0; i < specialPlantSaplings.length; i++) {
			if (specialPlantStages[i] > 0 && specialPlantStages[i] <= 3
					&& Server.getMinutesCounter() - specialPlantTimer[i] >= 5) {
				specialPlantStages[i]--;
				specialPlantTimer[i] = Server.getMinutesCounter();
				updateSpecialPlants();
				continue;
			}
			...
			long difference = Server.getMinutesCounter() - specialPlantTimer[i];
			...
			if (specialPlantTimer[i] == 0 || specialPlantState[i] == 2 || specialPlantState[i] == 3
					|| (state > nbStates)) {
				continue;
			}
			if (4 + state != specialPlantStages[i]
					&& specialPlantStages[i] <= specialPlantData.getEndingState() - specialPlantData.getStartingState()
							+ (specialPlantData == SpecialPlantData.SPIRIT_TREE ? 3 : -2)) {
				if (specialPlantStages[i] == specialPlantData.getEndingState() - specialPlantData.getStartingState()
						+ (specialPlantData == SpecialPlantData.SPIRIT_TREE ? 3 : -2)) {
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				specialPlantTimer[hopsFieldsData.getSpecialPlantsIndex()] = Server.getMinutesCounter();
				updateSpecialPlants();
				if (specialPlantStages[hopsFieldsData.getSpecialPlantsIndex()] == 3) {
					container.stop();
					return;
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if ((specialPlantStages[0] > 3 || specialPlantStages[2] > 3 || specialPlantStages[3] > 3)
			...
			player.getPA().sendMessage("You already have a spirit tree planted somewhere else.");
			return true;
		...
		CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				specialPlantTimer[specialPlantFieldData.getSpecialPlantsIndex()] = Server.getMinutesCounter();
	...
	public boolean harvestOrCheckHealth(int objectX, int objectY) {
		...
		final SpecialPlantData specialPlantData = SpecialPlantData
				.forId(specialPlantSaplings[specialPlantFieldsData.getSpecialPlantsIndex()]);
		if (specialPlantData == null) {
			return false;
		}
		if (!Constants.FARMING_ENABLED) {
		...
		if (specialPlantData == SpecialPlantData.SPIRIT_TREE
				&& specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] != 3) {
			handleSpiritTree();
			return true;
		}
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (specialPlantState[specialPlantFieldsData.getSpecialPlantsIndex()] == 3) {
					...
					specialPlantTimer[specialPlantFieldsData.getSpecialPlantsIndex()] = Server.getMinutesCounter()
							- specialPlantData.getGrowthTime();
					modifyStage(specialPlantFieldsData.getSpecialPlantsIndex());
					container.stop();
					return;
				}
				player.getPA().sendMessage("You harvest the crop, and pick some "
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] == 0) {
			player.getDialogue().sendStatement("This is one of the special patches. The soil has not been treated.",
			...
			CycleEventHandler.getInstance().addEvent(player, new CycleEvent() {
				...
				@Override
				public void execute(CycleEventContainer container) {
					if (specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()]
							- 4 < inspectData.getMessages().length - 2) {
						player.getDialogue().sendStatement(inspectData
								.getMessages()[specialPlantStages[specialPlantFieldsData.getSpecialPlantsIndex()] - 4]);
					} else if (specialPlantStages[specialPlantFieldsData
							.getSpecialPlantsIndex()] < specialPlantData.getEndingState()
