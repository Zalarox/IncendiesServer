...
public class GemCrafting extends GemData {
	...
	public enum gem_Data {
		 ...
		 */
		onyx(6585, 67, 167.5, 67, 115, 6575, 82, 120, 6577, 90, 165, 6579, 6581);
		...
		gem_Data(int baseElement, int cutLevel, double cutExp, int ringLevel, double ringExp, int ringFinal,
				int necklaceLevel, double necklaceExp, int necklaceFinal, int amuletLevel, double amuletExp,
				int midAmulet, int finalAmulet) {
	...
	public static void showInterface(Player player, int face) {
		...
		if (player.getInventory().getItemContainer().contains(face)) {
			for (int i = 0; i < GemData.craftInterfaceArray[interfaceType].length; i++) {
				player.getPA().sendUpdateItem(i, interfaceFrames[interfaceType][1],
						new Item(GemData.craftInterfaceArray[interfaceType][i], 1));
			}
			player.getPA().sendFrame246(interfaceFrames[interfaceType][0], 0, -1);
			player.getPA().sendString("Choose an item to make.", interfaceFrames[interfaceType][1] - 3);
		} else {
			player.getPA().sendFrame246(interfaceFrames[interfaceType][0], 120, 1595);
			player.getPA().sendString(interfaceMessage[interfaceType], interfaceFrames[interfaceType][1] - 3);
			for (int i = 0; i < GemData.craftInterfaceArray[interfaceType].length; i++) {
				player.getPA().sendUpdateItem(i, interfaceFrames[interfaceType][1], new Item(-1));
			}
		}
		player.getPA().sendString("What would you like to make?", 4226);
	}
	...
	public static void startCrafter(final Player player, final int gem, final int actionAmount, final int actionType) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (getData(gemType) == null
						|| !player.getInventory().getItemContainer().contains(getData(gemType).mapLocation[0])
						|| !player.getInventory().getItemContainer().contains(2357)) {
					container.stop();
					return;
				}
				container.setTick(4);
				...
				case 0:
					if (player.getSkill().getLevel()[Skill.CRAFTING] < getData(gemType).mapLocation[3]) {
						player.getDialogue().sendStatement(
								"You need a crafting level of " + getData(gemType).mapLocation[3] + " to craft this.");
						container.stop();
						return;
					}
					...
					player.getPA().sendMessage("You craft a ring.");
				...
				case 1:
					if (player.getSkill().getLevel()[Skill.CRAFTING] < getData(gemType).mapLocation[6]) {
						player.getDialogue().sendStatement(
								"You need a Crafting level of " + getData(gemType).mapLocation[6] + " to craft this.");
						container.stop();
						return;
					}
					if (getData(gemType).mapLocation[0] != 2357)
					...
					player.getPA().sendMessage("You craft a necklace.");
				...
				case 2:
					if (player.getSkill().getLevel()[Skill.CRAFTING] < getData(gemType).mapLocation[9]) {
						player.getDialogue().sendStatement(
								"You need a Crafting level of " + getData(gemType).mapLocation[9] + " to craft this.");
						container.stop();
						return;
					}
					if (getData(gemType).mapLocation[0] != 2357)
					...
					if (player.getInventory().getItemContainer().contains(1759)) {
						...
						player.getPA().sendMessage("You craft an amulet and attach a string to it.");
					} else {
						player.getInventory().addItem(new Item(getData(gemType).mapLocation[11], 1));
						player.getPA().sendMessage("You craft an amulet.");
					}
	...
	public static void string(final Player player, final int gemSlot) {
		if (!player.getInventory().getItemContainer().contains(1759)
				|| !player.getInventory().getItemContainer().contains(mendItems[gemSlot][0])) {
			return;
		}
		player.getInventory().removeItem(new Item(mendItems[gemSlot][0], 1));
		player.getInventory().removeItem(new Item(1759, 1));
		player.getInventory().addItem(new Item(mendItems[gemSlot][1], 1));
		player.getPA().sendMessage("You attach a string to the amulet.");
