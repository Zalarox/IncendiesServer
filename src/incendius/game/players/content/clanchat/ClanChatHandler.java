package incendius.game.players.content.clanchat;
...
import incendius.game.players.content.clanchat.load.ClanDataWriter;
...
public class ClanChatHandler {
		...
		if (clans[clanId] != null) {
			for (int j = 0; j < clans[clanId].members.length; j++) {
				if (clans[clanId].members[j] <= 0) {
					...
					Server.clanChat.clans[clanId].membersNumber += 1;
		...
		for (int i = 0; i < Constants.MAX_PLAYERS; i++) {
			if (PlayerHandler.players[i] != null) {
				if (PlayerHandler.players[i].playerName.equalsIgnoreCase(name)) {
					...
					Server.clanChat.clans[c.getInstance().clanId].addName(c2.playerName, -1);
					Server.clanChat.clans[c2.getInstance().clanId].membersNumber -= 1;
	...
	public void changeClanName(Player p, String name) {
		...
		for (int i = 0; i < clans.length; i++) {
			if (clans[i] != null) {
				if (Server.clanChat.clans[i].owner.equalsIgnoreCase(p.playerName)) {
					clans[i].name = name;
					...
					if (clans[clanId].members[i] > 0) {
						if (PlayerHandler.players[clans[clanId].members[i]] != null) {
							if (c.getInstance().clanId != -1) {
								if (Server.clanChat.clans[c.getInstance().clanId].owner
	...
	public void handleShareButton(Player p) {
		if (System.currentTimeMillis() - p.getInstance().clanDelay >= 1500) {
			if (p.getInstance().clanId >= 0
					&& (Server.clanChat.clans[p.getInstance().clanId].owner.equalsIgnoreCase(p.playerName)
							|| PlayerHandler.players[p.playerId].getInstance().playerRights == 2
							|| PlayerHandler.players[p.playerId].getInstance().playerRights == 3)) {
				boolean lootShareOn = Server.clanChat.clans[p.getInstance().clanId].lootshare == 1;
				boolean bothOff = Server.clanChat.clans[p.getInstance().clanId].lootshare == 0;
				if (bothOff && Server.clanChat.clans[p.getInstance().clanId].CSLS == 0) {
					Server.clanChat.clans[p.getInstance().clanId].CSLS = 1;
					Server.clanChat.clans[p.getInstance().clanId].lootshare = 1;
					Server.clanChat.sendToggleMessage(p.getInstance().clanId,
							"LootShare has been toggled to ON by the clan leader.", true);
					Server.clanChat.updateClanChat(p.getInstance().clanId);
					return;
				} else if (lootShareOn && Server.clanChat.clans[p.getInstance().clanId].CSLS == 1) {
					Server.clanChat.clans[p.getInstance().clanId].CSLS = 2;
					Server.clanChat.clans[p.getInstance().clanId].lootshare = 0;
					Server.clanChat.sendToggleMessage(p.getInstance().clanId,
							"LootShare has been toggled to OFF by the clan leader.", true);
					Server.clanChat.updateClanChat(p.getInstance().clanId);
					return;
				} else if (Server.clanChat.clans[p.getInstance().clanId].CSLS == 2) {
					if (Server.clanChat.clans[p.getInstance().clanId].membersNumber <= 1) {
						p.sendMessage("There must be at least two members in the channel to toggle CoinShare ON.");
						Server.clanChat.clans[p.getInstance().clanId].CSLS = 0;
						Server.clanChat.clans[p.getInstance().clanId].lootshare = 0;
						Server.clanChat.updateClanChat(p.getInstance().clanId);
						return;
					}
					Server.clanChat.clans[p.getInstance().clanId].CSLS = 3;
					Server.clanChat.clans[p.getInstance().clanId].lootshare = 2;
					Server.clanChat.sendToggleMessage(p.getInstance().clanId,
							"CoinShare has been toggled to ON by the clan leader.", false);
					Server.clanChat.updateClanChat(p.getInstance().clanId);
					return;
				} else if (Server.clanChat.clans[p.getInstance().clanId].lootshare == 2
						&& Server.clanChat.clans[p.getInstance().clanId].CSLS == 3) {
					Server.clanChat.clans[p.getInstance().clanId].CSLS = 0;
					Server.clanChat.clans[p.getInstance().clanId].lootshare = 0;
					Server.clanChat.sendToggleMessage(p.getInstance().clanId,
