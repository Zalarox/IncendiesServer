...
import incendius.action.Action;
...
import incendius.game.items.GameItem;
import incendius.game.items.ItemAssistant;
...
import incendius.game.npcs.data.SummoningAttacking;
import incendius.game.npcs.transformers.TransformHandler;
import incendius.game.objects.Objects;
import incendius.game.players.Logging;
import incendius.game.players.Boundaries.Area;
import incendius.game.players.actions.ActionHandler;
import incendius.game.players.actions.Bank;
import incendius.game.players.actions.DialogueHandler;
import incendius.game.players.actions.Food;
import incendius.game.players.actions.Potions;
import incendius.game.players.actions.TradeHandler;
import incendius.game.players.actions.combat.CombatAssistant;
import incendius.game.players.actions.combat.CombatPrayer;
import incendius.game.players.actions.combat.Curses;
import incendius.game.players.actions.combat.PlayerKilling;
import incendius.game.players.content.DwarfMultiCannon;
import incendius.game.players.content.InfernoAdze;
import incendius.game.players.content.Pouch;
import incendius.game.players.content.QuickCurses;
import incendius.game.players.content.QuickPrayers;
import incendius.game.players.content.SkillInterfaces;
import incendius.game.players.content.YellChat;
import incendius.game.players.content.clanchat.load.ClanProcessor;
import incendius.game.players.content.grandexchange.GrandExchange;
...
import incendius.game.players.content.minigames.impl.barrows.BarrowsConstants;
import incendius.game.players.content.skills.construction.BuildObject;
import incendius.game.players.content.skills.construction.ConstructionData;
import incendius.game.players.content.skills.construction.EnterHouse;
import incendius.game.players.content.skills.construction.FaceObject;
import incendius.game.players.content.skills.construction.KickPlayers;
import incendius.game.players.content.skills.construction.ObjectDeleting;
import incendius.game.players.content.skills.construction.ObjectLoading;
import incendius.game.players.content.skills.construction.Process;
import incendius.game.players.content.skills.cooking.Cooking;
import incendius.game.players.content.skills.dungeoneering.Party;
import incendius.game.players.content.skills.farming.Allotments;
import incendius.game.players.content.skills.farming.Bushes;
import incendius.game.players.content.skills.farming.Compost;
import incendius.game.players.content.skills.farming.Flowers;
import incendius.game.players.content.skills.farming.FruitTree;
import incendius.game.players.content.skills.farming.Herbs;
import incendius.game.players.content.skills.farming.Hops;
import incendius.game.players.content.skills.farming.Seedling;
import incendius.game.players.content.skills.farming.SpecialPlantOne;
import incendius.game.players.content.skills.farming.SpecialPlantTwo;
import incendius.game.players.content.skills.farming.ToolLeprechaun;
import incendius.game.players.content.skills.farming.WoodTrees;
import incendius.game.players.content.skills.prayer.Prayer;
import incendius.game.players.content.skills.runecrafting.Tiaras;
import incendius.game.players.content.skills.slayer.Slayer;
import incendius.game.players.content.skills.smithing.Smithing;
import incendius.game.players.content.skills.smithing.SmithingInterface;
import incendius.game.players.content.skills.summoning.Summoning;
import incendius.game.players.content.skills.thieving.Thieving;
import incendius.game.players.content.skills.woodcutting.Woodcutting;
import incendius.game.players.events.ConstitutionRestoreTask;
import incendius.game.players.events.SkillRestoreTask;
import incendius.game.players.events.SpecialRestoreTask;
import incendius.game.shops.ShopAssistant;
import incendius.handlers.Following;
import incendius.handlers.ProcessHandler;
import incendius.handlers.Skill;
import incendius.net.HostList;
import incendius.net.Packet;
import incendius.net.StaticPacketBuilder;
import incendius.util.ISAACRandomGen;
import incendius.util.Misc;
import incendius.util.Stream;
...
public class Player {
	...
	public void addAction(Action action) {
		...
		Server.getScheduler().schedule(currentAction);
	}
	...
	private void initializePlayerTasks() {
		Server.getScheduler().schedule(skillRestoreTask);
		Server.getScheduler().schedule(specialRestoreTask);
		Server.getScheduler().schedule(constitutionRestoreTask);
	}
	...
	public void overWildernessDitch(final int x, int y) {
		...
		if (y == 3523) {
			...
			turnPlayerTo(x, 3519);
			startAnimation(6132);
			Server.getScheduler().schedule(new Task(true) {
	...
	public void destruct() {
		 ...
		 */
		if (clanId >= 0) {
			Server.clanChat.leaveClan(playerId, clanId, true);
		}
	...
	public void dungemote() {
		Server.getScheduler().schedule(new Task(1) {
	...
	public void initialize() {
		...
		getPA().clearClanChat();
		if (savedClan != null)
			Server.clanChat.handleClanChatJoin(this, savedClan);
	...
	public int calculateMaxLP() {
		...
		Server.sendDeveloperNotice("Re-calculated your max life points, they are now " + calculatedLP + ".");
	...
	public void updateClanChatEditInterface(boolean update) {
		...
		for (int j = 0; j < Server.clanChat.clans.length; j++) {
			if (Server.clanChat.clans[j] != null) {
				if (Server.clanChat.clans[j].owner.equalsIgnoreCase(playerName)) {
					getPA().sendFrame126("", 47997);
					getPA().sendFrame126("" + Server.clanChat.clans[j].Recruits, 47997);
					getPA().sendFrame126("" + Server.clanChat.clans[j].Corporals, 47996);
					getPA().sendFrame126("" + Server.clanChat.clans[j].Sergeants, 47995);
					getPA().sendFrame126("" + Server.clanChat.clans[j].Lieutenants, 47994);
					getPA().sendFrame126("" + Server.clanChat.clans[j].Captains, 47993);
					getPA().sendFrame126("" + Server.clanChat.clans[j].Generals, 47992);

					cc = j;
				}
			}
		}
		getPA().sendFrame126(Server.clanChat.clans[cc].name, 47814);
		getPA().sendFrame126("[UPDATE]", 47998);
		getPA().showInterface(40172);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 0)
			getPA().sendFrame126("Anyone", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 1)
			getPA().sendFrame126("Any friends", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 2)
			getPA().sendFrame126("Recruit+", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 3)
			getPA().sendFrame126("Corporal+", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 4)
			getPA().sendFrame126("Sergeant+", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 5)
			getPA().sendFrame126("Lieutenant+", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 6)
			getPA().sendFrame126("Captain+", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 7)
			getPA().sendFrame126("General+", 47815);
		if (Server.clanChat.clans[cc].whoCanEnterChat == 8)
			getPA().sendFrame126("Only me", 47815);

		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 0)
			getPA().sendFrame126("Anyone", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 1)
			getPA().sendFrame126("Any friends", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 2)
			getPA().sendFrame126("Recruit+", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 3)
			getPA().sendFrame126("Corporal+", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 4)
			getPA().sendFrame126("Sergeant+", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 5)
			getPA().sendFrame126("Lieutenant+", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 6)
			getPA().sendFrame126("Captain+", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 7)
			getPA().sendFrame126("General+", 47816);
		if (Server.clanChat.clans[cc].whoCanTalkOnChat == 8)
			getPA().sendFrame126("Only me", 47816);

		if (Server.clanChat.clans[cc].whoCanKickOnChat == 2)
			getPA().sendFrame126("Recruit+", 47817);
		if (Server.clanChat.clans[cc].whoCanKickOnChat == 3)
			getPA().sendFrame126("Corporal+", 47817);
		if (Server.clanChat.clans[cc].whoCanKickOnChat == 4)
			getPA().sendFrame126("Sergeant+", 47817);
		if (Server.clanChat.clans[cc].whoCanKickOnChat == 5)
			getPA().sendFrame126("Lieutenant+", 47817);
		if (Server.clanChat.clans[cc].whoCanKickOnChat == 6)
			getPA().sendFrame126("Captain+", 47817);
		if (Server.clanChat.clans[cc].whoCanKickOnChat == 7)
			getPA().sendFrame126("General+", 47817);
		if (Server.clanChat.clans[cc].whoCanKickOnChat == 8)
			getPA().sendFrame126("Only me", 47817);
		if (update) {
			if (Server.clanChat.clans[cc].changesMade != true)
				sendMessage("Changes will take effect on your clan chat in the next 60 seconds.");
			Server.clanChat.clans[cc].changesMade = true;
		}
	}
