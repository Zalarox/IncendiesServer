...
import incendius.processors.GameProcessor;
...
public class Region {
	...
	public ArrayList<incendius.game.objects.Object> realObjects = new ArrayList<incendius.game.objects.Object>();
	...
	public void startItemProcess() {
		if (!itemProcessing) {
			GameProcessor.startItemProcess(this);
		}
	}
	...
	public static boolean objectExists(int id, int x, int y, int z) {
		...
		if (r == null)
			return false;
		for (incendius.game.objects.Object o : r.realObjects) {
			if (o.objectId == id) {
				if (o.objectX == x && o.objectY == y && o.height == z) {
					return true;
				}
			}
		}
	...
	public static boolean blockedShot(int x, int y, int height, int moveTypeX, int moveTypeY) {
		try {
			...
			if (moveTypeX == -1 && moveTypeY == 0)
				...
				return ((getClipping(x, y, height) & 0x20000) == 0
						&& (getClipping(checkX - 1, checkY, height) & 0x20000) == 0
						&& (getClipping(checkX - 1, checkY, height) & 0x20000) == 0);
			else if (moveTypeX == 1 && moveTypeY == -1)
				return ((getClipping(x, y, height) & 0x20000) == 0
						&& (getClipping(checkX + 1, checkY, height) & 0x20000) == 0
						&& (getClipping(checkX, checkY - 1, height) & 0x20000) == 0);
			else if (moveTypeX == -1 && moveTypeY == 1)
				return ((getClipping(x, y, height) & 0x20000) == 0
						&& (getClipping(checkX - 1, checkY, height) & 0x20000) == 0
						&& (getClipping(checkX, checkY + 1, height) & 0x20000) == 0);
	...
	public static void addObject(int objectId, int x, int y, int height, int type, int direction) {
		...
		r.realObjects.add(new incendius.game.objects.Object(objectId, x, y, height, direction, type));
		if (objectId < 0)
			return;
	...
	/*
	 * public static boolean isMembers(int x, int y, int height) { if(x >= 3272
	 * && x <= 3320 && y >= 2752 && y <= 2809) return false; if(x >= 2640 && x
	 * <= 2677 && y >= 2638 && y <= 2679) return false; int regionX = x >> 3;
	 * int regionY = y >> 3; int regionId = ((regionX / 8) << 8) + (regionY /
	 * 8); for (Region r : regions) { if (r.id() == regionId) { return
	 * r.members(); } } return false; }
	 */
